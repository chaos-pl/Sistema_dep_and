...
<head>
    ...
    <style>
        ...
        body.theme-dark .table { --bs-table-bg: transparent !important; --bs-table-color: #f8fafc !important; }
        body.theme-dark .table > :not(caption) > * > * { background-color: transparent !important; color: #f8fafc !important; border-bottom-color: rgba(255, 255, 255, 0.08) !important; }
        ...
        @media (prefers-color-scheme: dark) {
            ...
            body.theme-system .table { --bs-table-bg: transparent !important; --bs-table-color: #f8fafc !important; }
            body.theme-system .table > :not(caption) > * > * { background-color: transparent !important; color: #f8fafc !important; border-bottom-color: rgba(255, 255, 255, 0.08) !important; }
        }

        /* 5. EFECTO HOVER PREMIUM PARA TABLAS */
        ...
        /* ==============================================
           6. MENÚ LATERAL Y SCROLL OCULTO (CORRECCIÓN)
           ============================================== */
        .sidebar {
            width: 280px;
            height: 100vh; /* Altura estricta para evitar que se desborde */
        ...
        /* Sección de navegación (El medio scrolleable) */
        .sidebar-nav-container {
            ...
            /* Ocultar barra de scroll en IE 10+ */
    ...
    <aside class="sidebar d-none d-lg-flex">
        ...
        <div class="sidebar-header anime-sidebar-item">
            <div class="d-flex align-items-center gap-3 mb-2">
                <div class="bg-white text-primary rounded-4 d-flex align-items-center justify-content-center shadow-sm" style="width:48px;height:48px;flex-shrink:0;">
            ...
            <nav class="nav flex-column pb-3">
                @auth
                    @if(auth()->user()->hasRole('admin'))
                        <div class="small fw-bold text-uppercase opacity-50 mb-2 mt-1 ps-2" style="letter-spacing: 1px;">Gestión</div>
                        ...
                        @can('usuarios.ver')
                            <a href="{{ route('admin.expedientes-pendientes.index') }}" class="nav-link anime-sidebar-item {{ request()->routeIs('admin.expedientes-pendientes.*') ? 'active' : '' }}">
                                <i class="bi bi-person-exclamation"></i> Pendientes
                            </a>
                        @endcan
                        ...
                        <div class="small fw-bold text-uppercase opacity-50 mb-2 mt-3 ps-2" style="letter-spacing: 1px;">Escolar</div>
                        @can('usuarios.ver')
                            <a href="{{ route('admin.tutores.index') }}" class="nav-link anime-sidebar-item {{ request()->routeIs('admin.tutores.*') ? 'active' : '' }}">
                                <i class="bi bi-person-video3"></i> Tutores
                            </a>
                        @endcan

                        @can('grupos.ver')
                            <a href="{{ route('admin.grupos.index') }}" class="nav-link anime-sidebar-item {{ request()->routeIs('admin.grupos.*') ? 'active' : '' }}">
                                <i class="bi bi-collection-fill"></i> Grupos
                            </a>
                        @endcan

                        @can('carreras.ver')
                            <a href="{{ route('admin.carreras.index') }}" class="nav-link anime-sidebar-item {{ request()->routeIs('admin.carreras.*') ? 'active' : '' }}">
                                <i class="bi bi-mortarboard-fill"></i> Carreras
                            </a>
                        @endcan

                        <div class="small fw-bold text-uppercase opacity-50 mb-2 mt-3 ps-2" style="letter-spacing: 1px;">Seguridad</div>
                        @can('roles.ver')
                    ...
                    @elseif(auth()->user()->hasRole('estudiante'))
                        <div class="small fw-bold text-uppercase opacity-50 mb-2 mt-1 ps-2" style="letter-spacing: 1px;">Mi Cuenta</div>
                    ...
                    @elseif(auth()->user()->hasRole('tutor'))
                        ...
                        @can('grupos.ver.asignados')
                            <a href="{{ route('tutor.grupos.index') }}" class="nav-link anime-sidebar-item {{ request()->routeIs('tutor.grupos.*') ? 'active' : '' }}">
                                <i class="bi bi-people-fill"></i> Mis grupos
                            </a>
                        @endcan

                    @elseif(auth()->user()->hasRole('psicologo'))
                        <div class="small fw-bold text-uppercase opacity-50 mb-2 mt-1 ps-2" style="letter-spacing: 1px;">Clínica</div>
                    ...
                    <div class="small fw-bold text-uppercase opacity-50 mb-2 mt-3 ps-2" style="letter-spacing: 1px;">Ajustes</div>
                    @can('perfil.ver')
                ...
                <a href="{{ route('aviso.privacidad') }}" class="nav-link anime-sidebar-item {{ request()->routeIs('aviso.privacidad') ? 'active' : '' }}">
                    <i class="bi bi-shield-check"></i> Aviso legal
                </a>
            </nav>
        </div>
    ...
    <div class="offcanvas-body d-flex flex-column p-4 overflow-y-auto" style="scrollbar-width: none; -ms-overflow-style: none;">
        @auth
            <div class="d-flex align-items-center gap-3 mb-4">
                <div class="sidebar-user-icon" style="width:40px; height:40px; font-size:1rem;">
                    <i class="{{ $userAvatarClass }}"></i>
                </div>
                <div class="min-w-0">
        ...
        <nav class="nav flex-column gap-2 mb-4">
            @auth
                @if(auth()->user()->hasRole('admin'))
                    <div class="small fw-bold text-uppercase opacity-50 mb-1 mt-1 ps-2" style="letter-spacing: 1px;">Gestión</div>
                    <a href="{{ route('admin.dashboard') }}" class="nav-link text-white {{ request()->routeIs('admin.dashboard') ? 'bg-white bg-opacity-25' : 'bg-white bg-opacity-10' }} rounded-3 px-3 py-2">
                    ...
                    @can('usuarios.ver')<a href="{{ route('admin.expedientes-pendientes.index') }}" class="nav-link text-white opacity-75 px-3 py-2"><i class="bi bi-person-exclamation me-2"></i> Pendientes</a>@endcan
                    ...
                    <div class="small fw-bold text-uppercase opacity-50 mb-1 mt-3 ps-2" style="letter-spacing: 1px;">Escolar</div>
                    @can('usuarios.ver')<a href="{{ route('admin.tutores.index') }}" class="nav-link text-white opacity-75 px-3 py-2"><i class="bi bi-person-video3 me-2"></i> Tutores</a>@endcan
                    @can('grupos.ver')<a href="{{ route('admin.grupos.index') }}" class="nav-link text-white opacity-75 px-3 py-2"><i class="bi bi-folder-fill me-2"></i> Grupos</a>@endcan
                    @can('carreras.ver')<a href="{{ route('admin.carreras.index') }}" class="nav-link text-white opacity-75 px-3 py-2"><i class="bi bi-mortarboard-fill me-2"></i> Carreras</a>@endcan

                    <div class="small fw-bold text-uppercase opacity-50 mb-1 mt-3 ps-2" style="letter-spacing: 1px;">Seguridad</div>
                ...
                @elseif(auth()->user()->hasRole('tutor'))
                    ...
                    @can('grupos.ver.asignados')<a href="{{ route('tutor.grupos.index') }}" class="nav-link text-white opacity-75 px-3 py-2"><i class="bi bi-people-fill me-2"></i> Mis grupos</a>@endcan
            ...
            <a href="{{ route('aviso.privacidad') }}" class="nav-link text-white opacity-75 px-3 py-2"><i class="bi bi-shield-check me-2"></i> Aviso legal</a>
        </nav>
