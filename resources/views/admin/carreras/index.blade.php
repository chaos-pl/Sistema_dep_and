...
@section('content')
    <div class="app-card p-4 p-md-5 border-0 shadow-sm rounded-4">
        ...
        <div class="row align-items-center justify-content-between mb-5 gy-3">
            <div class="col-lg-5">
                <h4 class="fw-black text-body mb-1"><i class="bi bi-mortarboard-fill text-primary me-2"></i>Carreras Registradas</h4>
            ...
            <div class="col-lg-7 d-flex flex-column flex-sm-row justify-content-lg-end gap-3">
                <div class="position-relative w-100" style="max-width: 320px;">
                    ...
                    <input type="text" id="searchInput" class="form-control bg-body-tertiary rounded-pill border-0 shadow-sm search-input-prometeo" placeholder="Buscar carrera..." onkeyup="filterCarreras()">
                </div>

                @can('carreras.crear')
                    <a href="{{ route('admin.carreras.create') }}" class="btn btn-primary rounded-pill fw-bold shadow-sm d-flex align-items-center justify-content-center px-4" style="padding-top: 0.7rem; padding-bottom: 0.7rem;">
                    ...
                    <tr class="carrera-row border-bottom border-secondary border-opacity-10">
                        ...
                        <td class="text-end px-4 py-3 border-0">
                            <div class="d-flex justify-content-end gap-2">
                                @can('carreras.editar')
                                    <a href="{{ route('admin.carreras.edit', $carrera->id) }}" class="btn btn-sm btn-light border text-warning rounded-circle shadow-sm" style="width: 35px; height: 35px; display: inline-flex; align-items: center; justify-content: center;" title="Editar">
                                        <i class="bi bi-pencil-fill"></i>
                                    </a>
                                @endcan

                                @can('carreras.eliminar')
                                    <form action="{{ route('admin.carreras.destroy', $carrera->id) }}"
                                          method="POST"
                                          onsubmit="return confirm('ATENCIÓN: ¿Seguro que deseas eliminar esta carrera? Esta acción afectará a los grupos vinculados.');">
                                        @csrf
                                        @method('DELETE')
