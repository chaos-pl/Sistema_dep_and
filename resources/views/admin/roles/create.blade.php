...
@section('title', 'Crear Rol - PROMETEO')
@section('page-title', 'Nuevo Rol')
@section('page-subtitle', 'Registra un perfil y asigna sus capacidades')

@php
    // Mapeo de íconos para cada categoría de permisos
    $iconMap = [
        ...
        <form action="{{ route('admin.roles.store') }}" method="POST">
            ...
            <div class="row align-items-end border-bottom pb-4 mb-5">
                <div class="col-md-7 mb-4 mb-md-0">
                    <h4 class="fw-black mb-2 text-dark"><i class="bi bi-shield-plus text-primary me-2"></i>Definición del Rol</h4>
                    <p class="text-muted mb-0">Escribe el nombre del rol (ej. administrador, psicologo).</p>
                </div>
                <div class="col-md-5">
                    <input type="text" name="name" id="name" class="form-control form-control-lg bg-light" value="{{ old('name') }}" placeholder="Nombre del rol" required autofocus>
            ...
            <div class="mb-4">
                ...
                <div class="row g-4">
                    @foreach($permissions as $grupo => $items)
                        @php
                            // Obtenemos el ícono o asignamos uno genérico por defecto
                                ...
                                <div class="d-flex flex-column gap-2">
                                    @foreach($items as $permission)
                                        <div class="form-check custom-check">
                                            <input class="form-check-input shadow-none" type="checkbox" name="permissions[]" value="{{ $permission->id }}" id="perm_{{ $permission->id }}" {{ in_array($permission->id, old('permissions', [])) ? 'checked' : '' }}>
