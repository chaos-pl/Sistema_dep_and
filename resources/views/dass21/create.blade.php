...
@section('title', 'Aplicar DASS-21 - PROMETEO')
...
@push('styles')
    <style>
        ...
        .glass-badge {
            ...
            backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.3);
        }
        ...
        /* --- WIZARD (Paso a Paso) --- */
        .wizard-container { position: relative; min-height: 500px; overflow: hidden; }
        .question-step { position: absolute; top: 0; left: 0; width: 100%; visibility: hidden; opacity: 0; pointer-events: none; }
        .question-step.active { position: relative; visibility: visible; opacity: 1; pointer-events: auto; }

        /* --- BARRA DE PROGRESO --- */
        .progress-bar-container { height: 10px; background: rgba(124, 58, 237, 0.1); border-radius: 6px; overflow: hidden; margin-bottom: 2.5rem; }
        .progress-bar-fill { height: 100%; background: var(--app-primary); width: 0%; transition: width 0.5s cubic-bezier(0.25, 0.8, 0.25, 1); }

        .question-card { border: 1px solid rgba(148, 163, 184, 0.16); }

        /* --- DISEÑO DE PREGUNTAS Y REACTIVOS --- */
        .question-number {
            width: 60px; height: 60px; border-radius: 18px; display: inline-flex; align-items: center; justify-content: center;
            background: var(--app-primary); color: white; font-weight: 900; font-size: 1.5rem; box-shadow: 0 8px 20px rgba(124, 58, 237, 0.3);
        }
        .question-text { font-size: clamp(1.5rem, 3vw, 2.2rem); line-height: 1.35; letter-spacing: -0.5px; color: var(--app-text); }

        /* Cajas de opciones (Tiles) */
        .option-tile {
            border: 2px solid rgba(148, 163, 184, 0.25); border-radius: 24px; padding: 2rem 1rem; cursor: pointer;
            transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); background: var(--app-surface); min-height: 100%;
            display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center;
        }
        .reactivo-valor { font-size: 3.5rem; font-weight: 900; line-height: 1; margin-bottom: 0.75rem; color: var(--app-primary); transition: transform 0.3s ease; }
        .reactivo-texto { font-size: 1.1rem; font-weight: 700; color: var(--app-muted); transition: color 0.3s ease; }

        /* Efectos Hover */
        .option-tile:hover { border-color: var(--app-primary); transform: translateY(-4px); box-shadow: 0 12px 25px rgba(0,0,0,0.08); background-color: var(--app-primary-soft); }
        .option-tile:hover .reactivo-valor { transform: scale(1.1); }
        ...
        body.theme-dark .option-tile, body.theme-system .option-tile { background: rgba(255,255,255,0.03); border-color: rgba(255,255,255,0.1); }
        body.theme-dark .reactivo-texto, body.theme-system .reactivo-texto { color: #cbd5e1; }
        body.theme-dark .option-tile:hover, body.theme-system .option-tile:hover, body.theme-dark .option-tile.active, body.theme-system .option-tile.active { background: rgba(255,255,255,0.1); border-color: var(--app-primary); }
        body.theme-dark .option-tile.active .reactivo-texto, body.theme-system .option-tile.active .reactivo-texto { color: #ffffff; }
    </style>
@endpush
                ...
                <div class="row align-items-center banner-content">
                    <div class="col-lg-8">
                        ...
                        @if($hasRecentEvaluation)
                            <span class="badge bg-warning text-dark rounded-pill px-3 py-2 mb-3 ms-2 fw-bold shadow-sm" title="Realizar tamizajes con demasiada frecuencia puede alterar la precisión.">
                                <i class="bi bi-exclamation-triangle-fill me-1"></i> Evaluación reciente detectada
                            </span>
                        @endif
                            ...
                            <div class="question-step {{ $loop->first ? 'active' : '' }}" data-step="{{ $loop->iteration }}">
                                <div class="app-card question-card p-4 p-md-5 border-0 shadow-sm rounded-4">
                                    <div class="d-flex flex-column align-items-center text-center mb-5">
                                        <span class="question-number mb-4">{{ $question->item_number }}</span>
                                        <h2 class="fw-black question-text">{{ $question->statement }}</h2>
                                    </div>

                                    <div class="row justify-content-center g-3 g-md-4">
                                        @foreach([
                                            0 => 'No me ha ocurrido',
                                            1 => 'En algún grado / A veces',
